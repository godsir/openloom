...
use loom_app_server_client::EnvironmentManager;
use loom_app_server_client::ExecServerRuntimePaths;
use loom_app_server_client::InProcessAppServerClient;
use loom_app_server_client::InProcessClientStartArgs;
use loom_app_server_client::InProcessServerEvent;
use loom_app_server_protocol::ClientRequest;
use loom_app_server_protocol::ConfigWarningNotification;
use loom_app_server_protocol::JSONRPCErrorError;
use loom_app_server_protocol::McpServerElicitationAction;
use loom_app_server_protocol::McpServerElicitationRequestResponse;
...
use loom_app_server_protocol::ReviewTarget as ApiReviewTarget;
use loom_app_server_protocol::ServerNotification;
use loom_app_server_protocol::ServerRequest;
use loom_app_server_protocol::Thread as AppServerThread;
use loom_app_server_protocol::ThreadItem as AppServerThreadItem;
...
use loom_app_server_protocol::ThreadSortKey;
...
use loom_app_server_protocol::TurnStartedNotification;
...
use loom_config::ConfigLoadOptions;
use loom_config::LoaderOverrides;
use loom_config::format_config_error_with_source;
use loom_tui_stubs::StateDbHandle;
use loom_tui_stubs::check_execpolicy_for_warnings;
use loom_tui_stubs::config::Config;
use loom_tui_stubs::config::ConfigBuilder;
use loom_tui_stubs::config::ConfigOverrides;
use loom_tui_stubs::config::find_codex_home;
use loom_tui_stubs::config::load_config_as_toml_with_cli_and_load_options;
use loom_tui_stubs::config::resolve_oss_provider;
use loom_tui_stubs::config::resolve_profile_v2_config_path;
use loom_tui_stubs::find_thread_meta_by_name_str;
use loom_tui_stubs::format_exec_policy_error_with_source;
use loom_tui_stubs::path_utils;
use loom_tui_stubs::feedback::CodexFeedback;
...
use loom_tui_stubs::login::default_client::set_default_originator;
...
use loom_shim_stubs::otel::set_parent_from_context;
use loom_shim_stubs::otel::traceparent_context_from_env;
use loom_protocol::SessionId;
...
use loom_protocol::protocol::ReviewRequest;
use loom_protocol::protocol::ReviewTarget;
use loom_protocol::protocol::RolloutItem;
use loom_protocol::protocol::RolloutLine;
use loom_protocol::protocol::SessionConfiguredEvent;
...
use loom_cli_utils::SharedCliOptions;
...
pub async fn run_main(cli: Cli, arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    ...
    let otel = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        loom_tui_stubs::otel_init::build_provider(
            &config,
    ...
    loom_tui_stubs::otel_init::record_process_start(otel.as_ref(), "codex_exec");
    loom_tui_stubs::otel_init::install_sqlite_telemetry(otel.as_ref(), "codex_exec");
    ...
    let state_db = loom_tui_stubs::init_state_db(&config).await;
...
async fn run_exec_session(args: ExecRunArgs) -> anyhow::Result<()> {
    ...
    let (initial_operation, prompt_summary) = match (command.as_ref(), prompt, images) {
        (Some(ExecCommand::Review(review_cli)), _, _) => {
            ...
            let summary = loom_tui_stubs::review_prompts::user_facing_hint(&review_request.target);
    ...
    if !json_mode
        && let Some(message) =
            loom_tui_stubs::config::system_bwrap_warning(config.permissions.permission_profile())
    {
        ...
        match server_event {
            ...
            InProcessServerEvent::ServerNotification(mut notification) => {
                if let ServerNotification::Error(payload) = &notification {
                    ...
                    && matches!(
                        payload.turn.status,
                        loom_app_server_protocol::TurnStatus::Failed
                            | loom_app_server_protocol::TurnStatus::Interrupted
                    )
                {
                    error_seen = true;
                }
...
fn session_configured_from_thread_response(
    session_id: &str,
    thread_id: &str,
    thread_source: Option<loom_protocol::protocol::ThreadSource>,
    ...
    permission_profile: PermissionProfile,
    active_permission_profile: Option<loom_protocol::models::ActivePermissionProfile>,
    cwd: AbsolutePathBuf,
