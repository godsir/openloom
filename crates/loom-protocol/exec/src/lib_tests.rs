use super::*;
use loom_shim_stubs::otel::set_parent_from_w3c_trace_context;
...
fn exec_root_span_can_be_parented_from_trace_context() {
    ...
    let parent = loom_protocol::protocol::W3cTraceContext {
...
fn turn_items_for_thread_returns_matching_turn_items() {
    let thread = AppServerThread {
        ...
        source: loom_app_server_protocol::SessionSource::Exec,
        thread_source: None,
        agent_nickname: None,
        agent_role: None,
        git_info: None,
...
fn should_backfill_turn_completed_items_skips_ephemeral_threads() {
    let notification =
        ServerNotification::TurnCompleted(loom_app_server_protocol::TurnCompletedNotification {
...
async fn thread_start_params_include_review_policy_when_review_policy_is_manual_only() {
    ...
    assert_eq!(
        ...
        Some(loom_app_server_protocol::ApprovalsReviewer::User)
    );
...
async fn thread_start_params_include_review_policy_when_auto_review_is_enabled() {
    ...
    assert_eq!(
        params.approvals_reviewer,
        Some(loom_app_server_protocol::ApprovalsReviewer::AutoReview)
    );
}
...
async fn thread_start_params_include_user_thread_source() {
    ...
    assert_eq!(
        ...
        Some(loom_app_server_protocol::ThreadSource::User)
    );
}
...
async fn session_configured_from_thread_response_preserves_thread_source() {
    ...
    assert_eq!(
        event.thread_source,
        Some(loom_protocol::protocol::ThreadSource::User)
    );
}
...
fn sample_thread_start_response() -> ThreadStartResponse {
    ThreadStartResponse {
        thread: loom_app_server_protocol::Thread {
            ...
            thread_source: Some(loom_app_server_protocol::ThreadSource::User),
            agent_nickname: None,
            agent_role: None,
            git_info: None,
        ...
        sandbox: loom_app_server_protocol::SandboxPolicy::WorkspaceWrite {
            writable_roots: vec![],
