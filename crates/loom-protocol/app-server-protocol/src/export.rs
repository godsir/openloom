...
pub fn generate_json_with_experimental(out_dir: &Path, experimental_api: bool) -> Result<()> {
    ...
    write_pretty_json(
        out_dir.join("loom_app_server_protocol.schemas.json"),
        &bundle,
    )?;
    let flat_v2_bundle = build_flat_v2_schema(&bundle)?;
    write_pretty_json(
        out_dir.join("loom_app_server_protocol.v2.schemas.json"),
        &flat_v2_bundle,
    )?;
...
mod tests {
    ...
    fn schema_root() -> Result<PathBuf> {
        // TODO(Phase 3.1): Replace with loom-utils-cargo-bin::find_resource! once ported
        let schema_root = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("schema");
        Ok(schema_root)
    }
...
"#;
    ...
    fn generate_json_filters_experimental_fields_and_methods() -> Result<()> {
        ...
        let bundle_json =
            fs::read_to_string(output_dir.join("loom_app_server_protocol.schemas.json"))?;
        ...
        let flat_v2_bundle_json =
            fs::read_to_string(output_dir.join("loom_app_server_protocol.v2.schemas.json"))?;
        ...
        let flat_v2_bundle =
            read_json_value(&output_dir.join("loom_app_server_protocol.v2.schemas.json"))?;
