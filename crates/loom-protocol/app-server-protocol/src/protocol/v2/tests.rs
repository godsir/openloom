...
use loom_protocol::items::FileChangeItem;
use loom_protocol::items::ImageViewItem;
use loom_protocol::items::McpToolCallItem;
use loom_protocol::items::McpToolCallStatus as CoreMcpToolCallStatus;
use loom_protocol::items::ReasoningItem;
use loom_protocol::items::TurnItem;
use loom_protocol::items::UserMessageItem;
use loom_protocol::items::WebSearchItem;
use loom_protocol::mcp::CallToolResult;
use loom_protocol::memory_citation::MemoryCitation as CoreMemoryCitation;
use loom_protocol::memory_citation::MemoryCitationEntry as CoreMemoryCitationEntry;
use loom_protocol::models::AdditionalPermissionProfile as CoreAdditionalPermissionProfile;
use loom_protocol::models::BUILT_IN_PERMISSION_PROFILE_WORKSPACE;
use loom_protocol::models::FileSystemPermissions as CoreFileSystemPermissions;
use loom_protocol::models::ImageDetail;
use loom_protocol::models::MessagePhase;
use loom_protocol::models::NetworkPermissions as CoreNetworkPermissions;
use loom_protocol::models::WebSearchAction as CoreWebSearchAction;
use loom_protocol::permissions::FileSystemAccessMode as CoreFileSystemAccessMode;
use loom_protocol::permissions::FileSystemPath as CoreFileSystemPath;
use loom_protocol::permissions::FileSystemSandboxEntry as CoreFileSystemSandboxEntry;
use loom_protocol::permissions::FileSystemSpecialPath as CoreFileSystemSpecialPath;
use loom_protocol::protocol::AgentStatus as CoreAgentStatus;
use loom_protocol::protocol::AskForApproval as CoreAskForApproval;
use loom_protocol::protocol::GranularApprovalConfig as CoreGranularApprovalConfig;
...
fn sandbox_policy_round_trips_external_sandbox_network_access() {
    ...
    assert_eq!(
        core_policy,
        loom_protocol::protocol::SandboxPolicy::ExternalSandbox {
...
fn sandbox_policy_round_trips_read_only_network_access() {
    ...
    assert_eq!(
        core_policy,
        loom_protocol::protocol::SandboxPolicy::ReadOnly {
            network_access: true,
        }
    );
...
fn sandbox_policy_round_trips_workspace_write_access() {
    ...
    assert_eq!(
        core_policy,
        loom_protocol::protocol::SandboxPolicy::WorkspaceWrite {
            writable_roots: vec![],
            network_access: true,
            exclude_tmpdir_env_var: false,
            exclude_slash_tmp: false,
...
fn core_turn_item_into_thread_item_converts_supported_variants() {
    ...
    let file_change_item = TurnItem::FileChange(FileChangeItem {
        ...
        .into_iter()
        .collect(),
        status: Some(loom_protocol::protocol::PatchApplyStatus::Completed),
        auto_approved: None,
...
fn thread_settings_update_params_preserve_field_level_experimental_gates() {
    ...
    let collaboration_mode = ThreadSettingsUpdateParams {
        thread_id: "thread_123".to_string(),
        collaboration_mode: Some(loom_protocol::config_types::CollaborationMode {
            mode: loom_protocol::config_types::ModeKind::Plan,
            settings: loom_protocol::config_types::Settings {
