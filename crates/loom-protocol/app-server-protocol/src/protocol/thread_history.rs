...
use loom_protocol::items::parse_hook_prompt_message;
use loom_protocol::models::MessagePhase;
use loom_protocol::protocol::AgentReasoningEvent;
use loom_protocol::protocol::AgentReasoningRawContentEvent;
use loom_protocol::protocol::AgentStatus;
use loom_protocol::protocol::ApplyPatchApprovalRequestEvent;
use loom_protocol::protocol::CompactedItem;
use loom_protocol::protocol::ContextCompactedEvent;
use loom_protocol::protocol::DynamicToolCallResponseEvent;
use loom_protocol::protocol::ErrorEvent;
use loom_protocol::protocol::EventMsg;
use loom_protocol::protocol::ExecCommandBeginEvent;
use loom_protocol::protocol::ExecCommandEndEvent;
use loom_protocol::protocol::GuardianAssessmentEvent;
use loom_protocol::protocol::GuardianAssessmentStatus;
use loom_protocol::protocol::ImageGenerationBeginEvent;
use loom_protocol::protocol::ImageGenerationEndEvent;
use loom_protocol::protocol::ItemCompletedEvent;
use loom_protocol::protocol::ItemStartedEvent;
use loom_protocol::protocol::McpToolCallBeginEvent;
use loom_protocol::protocol::McpToolCallEndEvent;
use loom_protocol::protocol::PatchApplyBeginEvent;
use loom_protocol::protocol::PatchApplyEndEvent;
use loom_protocol::protocol::ReviewOutputEvent;
use loom_protocol::protocol::RolloutItem;
use loom_protocol::protocol::ThreadRolledBackEvent;
use loom_protocol::protocol::TurnAbortedEvent;
use loom_protocol::protocol::TurnCompleteEvent;
use loom_protocol::protocol::TurnStartedEvent;
use loom_protocol::protocol::UserMessageEvent;
use loom_protocol::protocol::ViewImageToolCallEvent;
use loom_protocol::protocol::WebSearchBeginEvent;
...
#[cfg(test)]
use loom_protocol::protocol::ExecCommandStatus as CoreExecCommandStatus;
#[cfg(test)]
use loom_protocol::protocol::PatchApplyStatus as CorePatchApplyStatus;
...
impl ThreadHistoryBuilder {
    ...
    fn handle_response_item(&mut self, item: &loom_protocol::models::ResponseItem) {
        let loom_protocol::models::ResponseItem::Message {
            role, content, id, ..
        } = item
        else {
            return;
    ...
    fn handle_item_started(&mut self, payload: &ItemStartedEvent) {
        match &payload.item {
            loom_protocol::items::TurnItem::Plan(plan) => {
            ...
            loom_protocol::items::TurnItem::UserMessage(_)
            | loom_protocol::items::TurnItem::HookPrompt(_)
            | loom_protocol::items::TurnItem::AgentMessage(_)
            | loom_protocol::items::TurnItem::Reasoning(_)
            | loom_protocol::items::TurnItem::WebSearch(_)
            | loom_protocol::items::TurnItem::ImageView(_)
            | loom_protocol::items::TurnItem::ImageGeneration(_)
            | loom_protocol::items::TurnItem::FileChange(_)
            | loom_protocol::items::TurnItem::McpToolCall(_)
            | loom_protocol::items::TurnItem::ContextCompaction(_) => {}
        }
    }
    ...
    fn handle_dynamic_tool_call_request(
        &mut self,
        payload: &loom_protocol::dynamic_tools::DynamicToolCallRequest,
    ) {
    ...
    fn handle_collab_agent_spawn_begin(
        &mut self,
        payload: &loom_protocol::protocol::CollabAgentSpawnBeginEvent,
    ) {
    ...
    fn handle_collab_agent_spawn_end(
        &mut self,
        payload: &loom_protocol::protocol::CollabAgentSpawnEndEvent,
    ) {
    ...
    fn handle_collab_agent_interaction_begin(
        &mut self,
        payload: &loom_protocol::protocol::CollabAgentInteractionBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_agent_interaction_end(
        &mut self,
        payload: &loom_protocol::protocol::CollabAgentInteractionEndEvent,
    ) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
    ...
    fn handle_collab_waiting_begin(
        &mut self,
        payload: &loom_protocol::protocol::CollabWaitingBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_waiting_end(
        &mut self,
        payload: &loom_protocol::protocol::CollabWaitingEndEvent,
    ) {
    ...
    fn handle_collab_close_begin(
        &mut self,
        payload: &loom_protocol::protocol::CollabCloseBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_close_end(&mut self, payload: &loom_protocol::protocol::CollabCloseEndEvent) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
            _ => CollabAgentToolCallStatus::Completed,
        };
    ...
    fn handle_collab_resume_begin(
        &mut self,
        payload: &loom_protocol::protocol::CollabResumeBeginEvent,
    ) {
        let item = ThreadItem::CollabAgentToolCall {
            id: payload.call_id.clone(),
    ...
    fn handle_collab_resume_end(
        &mut self,
        payload: &loom_protocol::protocol::CollabResumeEndEvent,
    ) {
        let status = match &payload.status {
            AgentStatus::Errored(_) | AgentStatus::NotFound => CollabAgentToolCallStatus::Failed,
            _ => CollabAgentToolCallStatus::Completed,
    ...
    fn handle_entered_review_mode(&mut self, payload: &loom_protocol::protocol::ReviewRequest) {
        let review = payload
            .user_facing_hint
            .clone()
    ...
    fn handle_exited_review_mode(
        &mut self,
        payload: &loom_protocol::protocol::ExitedReviewModeEvent,
    ) {
        let review = payload
            .review_output
            .as_ref()
...
fn convert_dynamic_tool_content_items(
    items: &[loom_protocol::dynamic_tools::DynamicToolCallOutputContentItem],
) -> Vec<DynamicToolCallOutputContentItem> {
    items
        .iter()
        .cloned()
        .map(|item| match item {
            loom_protocol::dynamic_tools::DynamicToolCallOutputContentItem::InputText { text } => {
                DynamicToolCallOutputContentItem::InputText { text }
            }
            loom_protocol::dynamic_tools::DynamicToolCallOutputContentItem::InputImage {
                image_url,
...
mod tests {
    ...
    use loom_protocol::items::TurnItem as CoreTurnItem;
    use loom_protocol::items::UserMessageItem as CoreUserMessageItem;
    use loom_protocol::items::build_hook_prompt_message;
    use loom_protocol::mcp::CallToolResult;
    use loom_protocol::models::ImageDetail;
    use loom_protocol::models::MessagePhase as CoreMessagePhase;
    use loom_protocol::models::WebSearchAction as CoreWebSearchAction;
    use loom_protocol::parse_command::ParsedCommand;
    use loom_protocol::protocol::AgentMessageEvent;
    use loom_protocol::protocol::AgentReasoningEvent;
    use loom_protocol::protocol::AgentReasoningRawContentEvent;
    use loom_protocol::protocol::ApplyPatchApprovalRequestEvent;
    use loom_protocol::protocol::CodexErrorInfo;
    use loom_protocol::protocol::CompactedItem;
    use loom_protocol::protocol::DynamicToolCallResponseEvent;
    use loom_protocol::protocol::ExecCommandEndEvent;
    use loom_protocol::protocol::ExecCommandSource;
    use loom_protocol::protocol::ItemStartedEvent;
    use loom_protocol::protocol::McpInvocation;
    use loom_protocol::protocol::McpToolCallEndEvent;
    use loom_protocol::protocol::PatchApplyBeginEvent;
    use loom_protocol::protocol::ThreadRolledBackEvent;
    ...
    fn reconstructs_dynamic_tool_items_from_request_and_response_events() {
        let events = vec![
            ...
            EventMsg::DynamicToolCallRequest(
                loom_protocol::dynamic_tools::DynamicToolCallRequest {
                    call_id: "dyn-1".into(),
                    turn_id: "turn-1".into(),
                    started_at_ms: 0,
        ...
        let events = vec![
            ...
            EventMsg::PatchApplyEnd(PatchApplyEndEvent {
                ...
                changes: [(
                    ...
                    loom_protocol::protocol::FileChange::Add {
                        content: "hello\n".into(),
                    },
                )]
                .into_iter()
    ...
    fn reconstructs_declined_guardian_command_item() {
        let events = vec![
            ...
            EventMsg::GuardianAssessment(GuardianAssessmentEvent {
                ...
                risk_level: Some(loom_protocol::protocol::GuardianRiskLevel::High),
                user_authorization: Some(loom_protocol::protocol::GuardianUserAuthorization::Low),
                rationale: Some("Would delete user data.".into()),
                decision_source: Some(
                    loom_protocol::protocol::GuardianAssessmentDecisionSource::Agent,
                ),
                action: serde_json::from_value(serde_json::json!({
                    "type": "command",
                    "source": "shell",
    ...
    fn reconstructs_collab_resume_end_item() {
        let events = vec![
            ...
            EventMsg::CollabResumeEnd(loom_protocol::protocol::CollabResumeEndEvent {
    ...
    fn reconstructs_collab_spawn_end_item_with_model_metadata() {
        ...
        let events = vec![
            ...
            EventMsg::CollabAgentSpawnEnd(loom_protocol::protocol::CollabAgentSpawnEndEvent {
                ...
                reasoning_effort: loom_protocol::openai_models::ReasoningEffort::Medium,
        ...
        assert_eq!(
            ...
            ThreadItem::CollabAgentToolCall {
                ...
                reasoning_effort: Some(loom_protocol::openai_models::ReasoningEffort::Medium),
                agents_states: [(
                    "00000000-0000-0000-0000-000000000002".into(),
                    CollabAgentState {
    ...
    fn reconstructs_interrupted_send_input_as_completed_collab_call() {
        ...
        let events = vec![
            ...
            EventMsg::CollabAgentInteractionBegin(
                loom_protocol::protocol::CollabAgentInteractionBeginEvent {
                    call_id: "send-1".into(),
                    started_at_ms: 0,
            ...
            EventMsg::CollabAgentInteractionEnd(
                loom_protocol::protocol::CollabAgentInteractionEndEvent {
                    call_id: "send-1".into(),
                    completed_at_ms: 0,
                    sender_thread_id: sender,
                    receiver_thread_id: receiver,
    ...
    fn ignores_plain_user_response_items_in_rollout_replay() {
        let items = vec![
            ...
            RolloutItem::ResponseItem(loom_protocol::models::ResponseItem::Message {
                id: Some("msg-1".into()),
                role: "user".into(),
                content: vec![loom_protocol::models::ContentItem::InputText {
