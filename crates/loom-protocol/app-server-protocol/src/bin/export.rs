...
fn main() -> Result<()> {
    let args = Args::parse();
    loom_app_server_protocol::generate_ts_with_options(
        &args.out_dir,
        args.prettier.as_deref(),
        loom_app_server_protocol::GenerateTsOptions {
            experimental_api: args.experimental,
            ..loom_app_server_protocol::GenerateTsOptions::default()
        },
    )?;
    loom_app_server_protocol::generate_json_with_experimental(&args.out_dir, args.experimental)
}
