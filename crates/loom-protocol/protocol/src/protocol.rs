    ...
    pub fn token_budget(&self) -> usize {
        match self {
            TruncationPolicy::Bytes(bytes) => {
                usize::try_from(loom_string::approx_tokens_from_byte_count(*bytes))
                    .unwrap_or(usize::MAX)
            }
...
mod tests {
    ...
    fn restricted_file_system_policy_treats_root_with_carveouts_as_scoped_access() {
        ...
        let canonical_cwd = loom_absolute_path::canonicalize_preserving_symlinks(cwd.path())
        ...
        let expected_blocked = AbsolutePathBuf::from_absolute_path(
            loom_absolute_path::canonicalize_preserving_symlinks(cwd.path())
                .expect("canonicalize cwd")
                .join("blocked"),
        )
