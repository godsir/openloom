...
fn augment_tool_spec_for_code_mode_preserves_exec_tool_description() {
    assert_eq!(
        augment_tool_spec_for_code_mode(ToolSpec::Freeform(FreeformTool {
            name: loom_shim_stubs::code_mode::PUBLIC_TOOL_NAME.to_string(),
...
fn tool_spec_to_code_mode_tool_definition_returns_augmented_nested_tools() {
    ...
    assert_eq!(
        ...
        Some(loom_shim_stubs::code_mode::ToolDefinition {
            name: "apply_patch".to_string(),
...
```"#
                .to_string(),
            kind: loom_shim_stubs::code_mode::CodeModeToolKind::Freeform,
            input_schema: None,
            output_schema: None,
        })
    );
