...
impl McpRuntimeContext {
    ...
    pub(crate) fn resolve_server_environment(
        &self,
        server_name: &str,
        config: &loom_config::McpServerConfig,
        ...
        if config.is_local_environment() {
            return match config.transport {
                loom_config::McpServerTransportConfig::Stdio { .. } => Err(format!(
                    "local stdio MCP server `{server_name}` requires a local environment"
                )),
                loom_config::McpServerTransportConfig::StreamableHttp { .. } => Ok(None),
            };
        }
...
fn ensure_remote_stdio_cwd(
    ...
    let loom_config::McpServerTransportConfig::Stdio { cwd, .. } = &config.transport else {
        return Ok(());
    };
...
pub(crate) fn emit_duration(metric: &str, duration: Duration, tags: &[(&str, &str)]) {
    if let Some(metrics) = loom_shim_stubs::otel::global() {
...
mod tests {
    ...
    use loom_config::DEFAULT_MCP_SERVER_ENVIRONMENT_ID;
