...
use loom_tui_stubs::resolve_installation_id;
use loom_exec_server::EnvironmentManager;
use loom_exec_server::ExecServerRuntimePaths;
use loom_shim_stubs::login::default_client::set_default_client_residency_requirement;
...
pub async fn run_main(
    ...
    let otel = loom_tui_stubs::otel_init::build_provider(
        &config,
    ...
    loom_tui_stubs::otel_init::record_process_start(otel.as_ref(), OTEL_SERVICE_NAME);
    loom_tui_stubs::otel_init::install_sqlite_telemetry(otel.as_ref(), OTEL_SERVICE_NAME);
...
mod tests {
    use super::*;
    use loom_config::types::OtelExporterKind;
    use loom_tui_stubs::config::ConfigBuilder;
    ...
    async fn mcp_server_builds_otel_provider_with_logs_traces_and_metrics() -> anyhow::Result<()> {
        ...
        let provider = loom_tui_stubs::otel_init::build_provider(
            &config,
            "0.0.0-test",
            Some(OTEL_SERVICE_NAME),
            DEFAULT_ANALYTICS_ENABLED,
