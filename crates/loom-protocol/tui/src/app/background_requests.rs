...
use loom_app_server_protocol::AppsListParams;
use loom_app_server_protocol::AppsListResponse;
use loom_app_server_protocol::MarketplaceAddParams;
use loom_app_server_protocol::MarketplaceAddResponse;
use loom_app_server_protocol::MarketplaceRemoveParams;
use loom_app_server_protocol::MarketplaceRemoveResponse;
use loom_app_server_protocol::MarketplaceUpgradeParams;
use loom_app_server_protocol::MarketplaceUpgradeResponse;
...
impl App {
    ...
    pub(super) fn handle_mcp_inventory_result(
        ...
        if config.mcp_servers.is_empty() && statuses.is_empty() {
            self.chat_widget
...
pub(super) async fn send_add_credits_nudge_email(
    request_handle: AppServerRequestHandle,
    credit_type: AddCreditsNudgeCreditType,
) -> Result<loom_app_server_protocol::AddCreditsNudgeEmailStatus> {
    let request_id = RequestId::String(format!("add-credits-nudge-{}", Uuid::new_v4()));
    let response: loom_app_server_protocol::SendAddCreditsNudgeEmailResponse = request_handle
    ...
    request_handle
        .request_typed(ClientRequest::ConfigBatchWrite {
            ...
            params: ConfigBatchWriteParams {
                edits: vec![loom_app_server_protocol::ConfigEdit {
...
pub(super) type McpInventoryMaps = (
    HashMap<String, loom_protocol::mcp::Tool>,
    HashMap<String, Vec<loom_protocol::mcp::Resource>>,
    HashMap<String, Vec<loom_protocol::mcp::ResourceTemplate>>,
    ...
    for status in statuses {
        ...
        auth_statuses.insert(
            ...
            match status.auth_status {
                loom_app_server_protocol::McpAuthStatus::Unsupported => McpAuthStatus::Unsupported,
                loom_app_server_protocol::McpAuthStatus::NotLoggedIn => McpAuthStatus::NotLoggedIn,
                loom_app_server_protocol::McpAuthStatus::BearerToken => McpAuthStatus::BearerToken,
                loom_app_server_protocol::McpAuthStatus::OAuth => McpAuthStatus::OAuth,
            },
        );
