...
use loom_app_server_protocol::AgentMessageDeltaNotification;
...
use loom_app_server_protocol::ItemStartedNotification;
use loom_app_server_protocol::JSONRPCErrorError;
use loom_app_server_protocol::McpServerElicitationRequest;
use loom_app_server_protocol::McpServerElicitationRequestParams;
use loom_app_server_protocol::McpServerStartupState;
use loom_app_server_protocol::McpServerStatusUpdatedNotification;
use loom_app_server_protocol::NetworkApprovalContext as AppServerNetworkApprovalContext;
use loom_app_server_protocol::NetworkApprovalProtocol as AppServerNetworkApprovalProtocol;
use loom_app_server_protocol::NetworkPolicyAmendment as AppServerNetworkPolicyAmendment;
use loom_app_server_protocol::NetworkPolicyRuleAction as AppServerNetworkPolicyRuleAction;
use loom_app_server_protocol::NonSteerableTurnKind as AppServerNonSteerableTurnKind;
use loom_app_server_protocol::PatchChangeKind;
use loom_app_server_protocol::PermissionsRequestApprovalParams;
...
use loom_app_server_protocol::ThreadClosedNotification;
use loom_app_server_protocol::ThreadItem;
use loom_app_server_protocol::ThreadSettings;
use loom_app_server_protocol::ThreadSettingsUpdatedNotification;
use loom_app_server_protocol::ThreadStartedNotification;
use loom_app_server_protocol::ThreadTokenUsage;
use loom_app_server_protocol::ThreadTokenUsageUpdatedNotification;
use loom_app_server_protocol::TokenUsageBreakdown;
use loom_app_server_protocol::ToolRequestUserInputParams;
...
use loom_app_server_protocol::UserInput as AppServerUserInput;
use loom_app_server_protocol::WarningNotification;
...
use loom_protocol::config_types::Settings;
...
async fn next_thread_settings_updated(
    ...
    for _ in 0..20 {
        ...
        if let loom_app_server_client::AppServerEvent::ServerNotification(
...
async fn handle_mcp_inventory_result_clears_committed_loading_cell() {
    ...
    app.handle_mcp_inventory_result(
        Ok(vec![McpServerStatus {
            ...
            auth_status: loom_app_server_protocol::McpAuthStatus::Unsupported,
        }]),
...
async fn enqueue_primary_thread_session_replays_turns_before_initial_prompt_submit() -> Result<()> {
    ...
    app.chat_widget = ChatWidget::new_with_app_event(ChatWidgetInit {
        ...
        feedback: loom_tui_stubs::feedback::CodexFeedback::new(),
        is_first_run: false,
...
async fn collab_receiver_notification_does_not_cache_not_found_thread() {
    ...
    app.handle_thread_event_now(ThreadBufferedEvent::Notification(
        ServerNotification::ItemCompleted(loom_app_server_protocol::ItemCompletedNotification {
            thread_id: ThreadId::new().to_string(),
            turn_id: "turn-1".to_string(),
            completed_at_ms: 0,
            item: ThreadItem::CollabAgentToolCall {
                id: "send-1".to_string(),
                tool: loom_app_server_protocol::CollabAgentTool::SendInput,
                status: loom_app_server_protocol::CollabAgentToolCallStatus::Failed,
                sender_thread_id: ThreadId::new().to_string(),
                receiver_thread_ids: vec![receiver_thread_id.to_string()],
                ...
                agents_states: HashMap::from([(
                    ...
                    loom_app_server_protocol::CollabAgentState {
                        status: loom_app_server_protocol::CollabAgentStatus::NotFound,
                        message: None,
                    },
                )]),
            },
...
fn update_memory_settings_updates_current_thread_memory_mode() -> Result<()> {
    ...
    runtime.block_on(async {
        ...
        let state_db = loom_tui_stubs::state::StateRuntime::init(
...
async fn inactive_thread_exec_approval_preserves_context() {
    ...
    assert_eq!(
        available_decisions,
        vec![
            loom_app_server_protocol::CommandExecutionApprovalDecision::Accept,
            loom_app_server_protocol::CommandExecutionApprovalDecision::AcceptForSession,
            loom_app_server_protocol::CommandExecutionApprovalDecision::ApplyNetworkPolicyAmendment {
                network_policy_amendment: AppServerNetworkPolicyAmendment {
                    host: "example.com".to_string(),
                    action: AppServerNetworkPolicyRuleAction::Allow,
                },
            },
            loom_app_server_protocol::CommandExecutionApprovalDecision::Cancel,
        ]
    );
}
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ItemStarted(ItemStartedNotification {
            ...
            item: ThreadItem::FileChange {
                ...
                status: loom_app_server_protocol::PatchApplyStatus::InProgress,
            },
        }),
    )
    .await
...
async fn inactive_thread_permissions_approval_preserves_file_system_permissions() {
    ...
    let request = ServerRequest::PermissionsRequestApproval {
        ...
        params: PermissionsRequestApprovalParams {
            ...
            permissions: loom_app_server_protocol::RequestPermissionProfile {
                network: Some(AdditionalNetworkPermissions {
                    enabled: Some(true),
                }),
                file_system: Some(AdditionalFileSystemPermissions {
    ...
    assert_matches!(
        ...
        Ok(AppEvent::SubmitThreadOp {
            ...
            op: Op::ResolveElicitation {
                server_name,
                request_id: AppServerRequestId::Integer(10),
                decision: loom_app_server_protocol::McpServerElicitationAction::Decline,
                content: None,
                meta: None,
            },
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ThreadStarted(ThreadStartedNotification {
            thread: Thread {
                ...
                source: loom_app_server_protocol::SessionSource::Unknown,
                thread_source: None,
                agent_nickname: Some("Robie".to_string()),
                agent_role: Some("explorer".to_string()),
                git_info: None,
...
async fn side_parent_status_prioritizes_input_over_approval() -> Result<()> {
    ...
    app.enqueue_thread_notification(
        ...
        ServerNotification::ServerRequestResolved(
            loom_app_server_protocol::ServerRequestResolvedNotification {
...
async fn side_thread_ignores_global_mcp_startup_notifications() {
    ...
    app.handle_app_server_event(
        &app_server,
        loom_app_server_client::AppServerEvent::ServerNotification(
...
async fn active_non_primary_shutdown_target_returns_none_for_non_shutdown_event() -> Result<()> {
    ...
    assert_eq!(
        app.active_non_primary_shutdown_target(&ServerNotification::SkillsChanged(
            loom_app_server_protocol::SkillsChangedNotification {},
        )),
        None
    );
    Ok(())
...
async fn clear_ui_header_shows_fast_status_for_fast_capable_models() {
    ...
    app.chat_widget.set_service_tier(Some(
        loom_protocol::config_types::ServiceTier::Fast
            .request_value()
            .to_string(),
    ));
    ...
    Box::pin(async {
        ...
        let op = AppCommand::override_turn_context(
            ...
            Some(ActivePermissionProfile::new(
                loom_protocol::models::BUILT_IN_PERMISSION_PROFILE_WORKSPACE,
            )),
            /*windows_sandbox_level*/ None,
...
async fn inactive_thread_settings_notification_updates_cached_collaboration_mode() {
    ...
    let notification = ThreadSettingsUpdatedNotification {
        ...
        thread_settings: ThreadSettings {
            ...
            approval_policy: AskForApproval::OnRequest,
            approvals_reviewer: loom_app_server_protocol::ApprovalsReviewer::AutoReview,
            sandbox_policy: loom_app_server_protocol::SandboxPolicy::ReadOnly {
                network_access: false,
            },
            active_permission_profile: Some(
                loom_app_server_protocol::ActivePermissionProfile::read_only(),
            ),
            model: "gpt-plan".to_string(),
            model_provider: "openai".to_string(),
            service_tier: None,
