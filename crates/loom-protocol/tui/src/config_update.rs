...
use loom_app_server_protocol::ConfigEdit;
use loom_app_server_protocol::ConfigReadParams;
...
use loom_app_server_protocol::SkillsConfigWriteParams;
    ...
    let service_tier_edit = service_tier.map_or_else(
        ...
        |service_tier| {
            let config_value = if service_tier == SERVICE_TIER_DEFAULT_REQUEST_VALUE {
                ...
                match loom_protocol::config_types::ServiceTier::from_request_value(service_tier) {
                    Some(loom_protocol::config_types::ServiceTier::Fast) => "fast",
                    Some(loom_protocol::config_types::ServiceTier::Flex) => "flex",
                    None => service_tier,
                }
            };
