...
impl HooksBrowserView {
    ...
    pub(crate) fn new(
        ...
        warnings: Vec<String>,
        errors: Vec<loom_app_server_protocol::HookErrorInfo>,
        app_event_tx: AppEventSender,
    ) -> Self {
        Self::from_entry(
            HooksListEntry {
    ...
    fn event_rows(&self) -> Vec<EventRow> {
        loom_protocol::protocol::HookEventName::iter()
            .map(|event_name| {
    ...
    fn selected_event(&self) -> Option<HookEventName> {
        self.state
            .selected_idx
            .and_then(|idx| loom_protocol::protocol::HookEventName::iter().nth(idx))
            .map(Into::into)
    }
    ...
    fn page_len(&self) -> usize {
        match self.page {
            HooksBrowserPage::Events => loom_protocol::protocol::HookEventName::iter().count(),
...
mod tests {
    ...
    use loom_app_server_protocol::HookErrorInfo;
    use loom_app_server_protocol::HookEventName;
