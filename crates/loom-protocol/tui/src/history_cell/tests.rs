...
use loom_config::types::McpServerConfig;
use loom_config::types::McpServerDisabledReason;
use loom_otel_stub::RuntimeMetricTotals;
...
fn multiline_command_wraps_with_extra_indent_on_subsequent_lines() {
    ...
    let cmd = "set -o pipefail\ncargo test -p loom-tui --quiet".to_string();
    let call_id = "c1".to_string();
    let mut cell = ExecCell::new(
        ExecCall {
            call_id: call_id.clone(),
