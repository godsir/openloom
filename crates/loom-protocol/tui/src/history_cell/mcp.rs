...
pub(crate) struct McpToolCallCell {
    ...
    result: Option<Result<loom_protocol::mcp::CallToolResult, String>>,
...
impl McpToolCallCell {
    ...
    pub(crate) fn complete(
        &mut self,
        duration: Duration,
        result: Result<loom_protocol::mcp::CallToolResult, String>,
...
impl HistoryCell for McpToolCallCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        if let Some(result) = &self.result {
            match result {
                Ok(loom_protocol::mcp::CallToolResult { content, .. }) => {
                    if !content.is_empty() {
...
pub(crate) fn new_mcp_tools_output(
    config: &Config,
    tools: HashMap<String, loom_protocol::mcp::Tool>,
