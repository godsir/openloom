...
use clap::Parser;
use loom_cli_utils::ApprovalModeCliArg;
...
pub struct Cli {
    ...
    // Internal controls set by the top-level `loom resume` subcommand.
    // These are not exposed as user flags on the base `loom` command.
    #[clap(skip)]
    ...
    /// top-level `loom resume <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
    ...
    // Internal controls set by the top-level `loom fork` subcommand.
    // These are not exposed as user flags on the base `loom` command.
    #[clap(skip)]
    ...
    /// top-level `loom fork <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
