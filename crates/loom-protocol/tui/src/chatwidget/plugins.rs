...
use loom_app_server_protocol::PluginDetail;
...
use loom_app_server_protocol::PluginSummary;
use loom_app_server_protocol::PluginUninstallResponse;
use loom_core_plugins::OPENAI_CURATED_MARKETPLACE_NAME;
            ...
            let actions: Vec<SelectionAction> = if let Some(marketplace_path) = marketplace_path {
                vec![Box::new(move |tx| {
                    ...
                    tx.send(AppEvent::FetchPluginDetail {
                        cwd: cwd.clone(),
                        params: loom_app_server_protocol::PluginReadParams {
...
fn plugin_hook_summary(plugin: &PluginDetail) -> String {
    if plugin.hooks.is_empty() {
        ...
        let mut event_counts = Vec::<(loom_app_server_protocol::HookEventName, usize)>::new();
