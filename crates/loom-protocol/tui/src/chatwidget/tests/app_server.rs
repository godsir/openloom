...
fn thread_settings_for_test(
    model: &str,
    thread_id: ThreadId,
) -> loom_app_server_protocol::ThreadSettingsUpdatedNotification {
    loom_app_server_protocol::ThreadSettingsUpdatedNotification {
        thread_id: thread_id.to_string(),
        thread_settings: loom_app_server_protocol::ThreadSettings {
...
async fn invalid_url_elicitation_is_declined() {
    ...
    chat.handle_elicitation_request_now(
        loom_app_server_protocol::RequestId::Integer(9),
        loom_app_server_protocol::McpServerElicitationRequestParams {
            thread_id: request_thread_id.to_string(),
            turn_id: Some("turn-auth".to_string()),
            server_name: "payments".to_string(),
            request: loom_app_server_protocol::McpServerElicitationRequest::Url {
                meta: None,
    ...
    assert_matches!(
        ...
        Ok(AppEvent::SubmitThreadOp {
            ...
            op: Op::ResolveElicitation {
                server_name,
                request_id: loom_app_server_protocol::RequestId::Integer(9),
...
async fn thread_settings_updated_updates_visible_state_without_transcript() {
    ...
    assert_eq!(
        chat.config_ref()
            ...
            .id,
        loom_protocol::models::BUILT_IN_PERMISSION_PROFILE_READ_ONLY
    );
