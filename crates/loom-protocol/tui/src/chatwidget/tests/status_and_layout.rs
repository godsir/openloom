...
async fn ctrl_c_interrupt_pauses_active_goal_turn() {
    ...
    let mut goal = test_thread_goal(
        loom_app_server_protocol::ThreadGoalStatus::Active,
    ...
    chat.handle_server_notification(
        ServerNotification::ThreadGoalUpdated(
            loom_app_server_protocol::ThreadGoalUpdatedNotification {
...
async fn ambient_pet_screen_bottom_anchor_uses_terminal_bottom() {
    use loom_config::types::TuiPetAnchor;
    use ratatui::layout::Rect;
...
async fn interrupted_turn_clears_visible_running_hook() {
    ...
    handle_hook_started(
        ...
        hook_started_run(
            ...
            loom_app_server_protocol::HookEventName::PreToolUse,
...
async fn renamed_thread_footer_title_snapshot() {
    ...
    chat.handle_server_notification(
        ServerNotification::ThreadNameUpdated(
            loom_app_server_protocol::ThreadNameUpdatedNotification {
                thread_id: thread_id.to_string(),
...
async fn status_line_goal_complete_elapsed_footer_snapshot() {
    ...
    chat.refresh_status_line();
    let mut goal = test_thread_goal(
        loom_app_server_protocol::ThreadGoalStatus::Complete,
        /*token_budget*/ None,
        /*tokens_used*/ 40_000,
    );
...
async fn thread_goal_update_for_other_thread_is_ignored() {
    ...
    let mut goal = test_thread_goal(
        loom_app_server_protocol::ThreadGoalStatus::BudgetLimited,
        /*token_budget*/ Some(50_000),
        /*tokens_used*/ 50_000,
    );
...
fn goal_status_indicator_formats_statuses_and_budgets() {
    ...
    assert_eq!(
        goal_status_indicator_from_app_goal(&test_thread_goal(
            loom_app_server_protocol::ThreadGoalStatus::Blocked,
            /*token_budget*/ None,
            /*tokens_used*/ 0,
        )),
    ...
    assert_eq!(
        goal_status_indicator_from_app_goal(&test_thread_goal(
            loom_app_server_protocol::ThreadGoalStatus::UsageLimited,
            /*token_budget*/ None,
            /*tokens_used*/ 0,
        )),
...
fn test_thread_goal(
    status: loom_app_server_protocol::ThreadGoalStatus,
    token_budget: Option<i64>,
    tokens_used: i64,
) -> loom_app_server_protocol::ThreadGoal {
    loom_app_server_protocol::ThreadGoal {
        thread_id: "thread-1".to_string(),
...
async fn post_tool_use_hook_events_render_snapshot() {
    assert_hook_events_snapshot(
        loom_app_server_protocol::HookEventName::PostToolUse,
...
async fn blocked_and_failed_hooks_render_feedback_and_errors() {
    ...
    handle_hook_completed(
        ...
        hook_completed_run(
            ...
            vec![loom_app_server_protocol::HookOutputEntry {
                kind: loom_app_server_protocol::HookOutputEntryKind::Feedback,
    ...
    handle_hook_completed(
        ...
        hook_completed_run(
            ...
            vec![loom_app_server_protocol::HookOutputEntry {
                kind: loom_app_server_protocol::HookOutputEntryKind::Error,
...
async fn completed_same_id_hook_output_survives_restart() {
    ...
    handle_hook_started(
        ...
        hook_started_run(
            hook_id,
            loom_app_server_protocol::HookEventName::Stop,
    ...
    handle_hook_completed(
        ...
        hook_completed_run(
            ...
            vec![loom_app_server_protocol::HookOutputEntry {
                kind: loom_app_server_protocol::HookOutputEntryKind::Stop,
...
async fn hook_completed_before_reveal_renders_completed_without_running_flash() {
    ...
    handle_hook_started(
        ...
        hook_started_run(
            ...
            loom_app_server_protocol::HookEventName::SessionStart,
...
fn hook_run_summary(
    ...
    loom_app_server_protocol::HookRunSummary {
        ...
        started_at: 1,
        completed_at: (status != loom_app_server_protocol::HookRunStatus::Running).then_some(2),
        duration_ms: (status != loom_app_server_protocol::HookRunStatus::Running).then_some(1),
        entries,
    }
}
