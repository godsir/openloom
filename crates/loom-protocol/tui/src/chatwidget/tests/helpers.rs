...
pub(super) async fn make_chatwidget_manual(
    ...
    widget.normal_placeholder_text = "Ask Loom to do anything".to_string();
...
fn token_usage_breakdown(usage: TokenUsage) -> loom_app_server_protocol::TokenUsageBreakdown {
    loom_app_server_protocol::TokenUsageBreakdown {
            ...
            chat.handle_server_notification(
                ServerNotification::ThreadTokenUsageUpdated(
                    loom_app_server_protocol::ThreadTokenUsageUpdatedNotification {
                        ...
                        token_usage: loom_app_server_protocol::ThreadTokenUsage {
...
pub(super) fn begin_exec_with_source(
    ...
    let command_actions = loom_shell_command::parse_command::parse_command(&command)
        .into_iter()
    ...
    let item = AppServerThreadItem::CommandExecution {
        ...
        command: loom_shell_command::parse_command::shlex_join(&command),
...
pub(super) fn terminal_interaction(
    ...
    chat.handle_server_notification(
        ServerNotification::TerminalInteraction(
            loom_app_server_protocol::TerminalInteractionNotification {
                thread_id: thread_id(chat),
                turn_id: chat
                    .turn_lifecycle
                    .last_turn_id
...
pub(super) fn plugins_test_detail(
    ...
    description: Option<&str>,
    skills: &[&str],
    hooks: &[(loom_app_server_protocol::HookEventName, usize)],
    apps: &[(&str, bool)],
    mcp_servers: &[&str],
) -> PluginDetail {
    PluginDetail {
        ...
        hooks: hooks
            ...
            .flat_map(|(event_index, (event_name, handler_count))| {
                (0..*handler_count).map(move |handler_index| {
                    loom_app_server_protocol::PluginHookSummary {
...
pub(super) fn hook_run(
    run_id: &str,
    event_name: loom_app_server_protocol::HookEventName,
    status: loom_app_server_protocol::HookRunStatus,
    status_message: &str,
    entries: Vec<loom_app_server_protocol::HookOutputEntry>,
) -> loom_app_server_protocol::HookRunSummary {
    loom_app_server_protocol::HookRunSummary {
        id: run_id.to_string(),
        event_name,
        handler_type: loom_app_server_protocol::HookHandlerType::Command,
        execution_mode: loom_app_server_protocol::HookExecutionMode::Sync,
        scope: loom_app_server_protocol::HookScope::Turn,
        source_path: PathBuf::from(test_path_display("/tmp/hooks.json")).abs(),
        source: loom_app_server_protocol::HookSource::User,
        display_order: 0,
        status,
        ...
        completed_at: matches!(
            status,
            loom_app_server_protocol::HookRunStatus::Completed
                | loom_app_server_protocol::HookRunStatus::Failed
                | loom_app_server_protocol::HookRunStatus::Blocked
                | loom_app_server_protocol::HookRunStatus::Stopped
        )
        .then_some(11),
        duration_ms: matches!(
            status,
...
pub(super) async fn assert_hook_events_snapshot(
    ...
    handle_hook_started(
        ...
        hook_run(
            run_id,
            event_name,
            loom_app_server_protocol::HookRunStatus::Running,
            status_message,
            Vec::new(),
        ),
    );
    ...
    handle_hook_completed(
        ...
        hook_run(
            ...
            status_message,
            vec![
                loom_app_server_protocol::HookOutputEntry {
                    kind: loom_app_server_protocol::HookOutputEntryKind::Warning,
                    text: "Heads up from the hook".to_string(),
                },
                loom_app_server_protocol::HookOutputEntry {
                    kind: loom_app_server_protocol::HookOutputEntryKind::Context,
...
fn hook_event_label(event_name: loom_app_server_protocol::HookEventName) -> &'static str {
    match event_name {
        loom_app_server_protocol::HookEventName::PreToolUse => "PreToolUse",
        loom_app_server_protocol::HookEventName::PermissionRequest => "PermissionRequest",
        loom_app_server_protocol::HookEventName::PostToolUse => "PostToolUse",
        loom_app_server_protocol::HookEventName::PreCompact => "PreCompact",
        loom_app_server_protocol::HookEventName::PostCompact => "PostCompact",
        loom_app_server_protocol::HookEventName::SessionStart => "SessionStart",
        loom_app_server_protocol::HookEventName::UserPromptSubmit => "UserPromptSubmit",
        loom_app_server_protocol::HookEventName::SubagentStart => "SubagentStart",
        loom_app_server_protocol::HookEventName::SubagentStop => "SubagentStop",
        loom_app_server_protocol::HookEventName::Stop => "Stop",
    }
}
