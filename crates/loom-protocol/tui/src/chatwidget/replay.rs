            ...
            if matches!(
                ...
                self.handle_turn_completed_notification(
                    TurnCompletedNotification {
                        ...
                        turn: Turn {
                            id: turn_id,
                            items_view: loom_app_server_protocol::TurnItemsView::NotLoaded,
                            items: Vec::new(),
                            status,
                            error,
                            started_at,
    ...
    pub(super) fn handle_thread_item(
        ...
        match item {
                ...
                self.on_agent_message_item_completed(
                    AgentMessageItem {
                        ...
                        memory_citation: memory_citation.map(|citation| {
                            loom_protocol::memory_citation::MemoryCitation {
                                entries: citation
                                    .entries
                                    .into_iter()
                                    .map(|entry| {
                                        loom_protocol::memory_citation::MemoryCitationEntry {
                                            path: entry.path,
            ...
            item @ ThreadItem::CommandExecution {
                status: loom_app_server_protocol::CommandExecutionStatus::InProgress,
                ..
            ...
            item @ ThreadItem::McpToolCall {
                status: loom_app_server_protocol::McpToolCallStatus::InProgress,
                ..
            ...
            ThreadItem::WebSearch { id, query, action } => {
                ...
                self.on_web_search_end(
                    id,
                    query,
                    action.unwrap_or(loom_app_server_protocol::WebSearchAction::Other),
                );
            }
