...
pub mod otel_init {
    ...
    impl OtelProvider {
        pub fn logger_layer<S>(
            &self,
        ) -> Option<impl tracing_subscriber::layer::Layer<S> + Send + Sync + 'static>
        where
...
pub mod windows_sandbox {
    ...
    pub trait WindowsSandboxLevelExt {
        fn from_config(
            config: &loom_tui_stubs::config::Config,
        ) -> loom_protocol::config_types::WindowsSandboxLevel;
    }
...
// ─── util ───
pub mod util {
    pub fn normalize_thread_name(name: &str) -> String {
        name.to_string()
    }
}
...
pub fn web_search_detail() -> String {
    String::new()
}
