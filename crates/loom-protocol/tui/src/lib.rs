...
use loom_app_server_client::RemoteAppServerClient;
use loom_app_server_client::RemoteAppServerConnectArgs;
pub use loom_app_server_client::RemoteAppServerEndpoint;
use loom_app_server_protocol::Account as AppServerAccount;
use loom_app_server_protocol::AskForApproval;
use loom_app_server_protocol::AuthMode as AppServerAuthMode;
use loom_app_server_protocol::ConfigWarningNotification;
use loom_app_server_protocol::Thread as AppServerThread;
use loom_app_server_protocol::ThreadListCwdFilter;
...
mod legacy_core;
// pub(crate) use loom_app_server_client::legacy_core; // replaced by local module
    ...
    match app_server_target {
        AppServerTarget::Embedded => state_db::try_init(config).await.map(Some).map_err(|err| {
            std::io::Error::other(LocalStateDbStartupError::new(
                loom_tui_stubs::state::state_db_path(config.sqlite_home.as_path()),
                err.to_string(),
            ))
        }),
...
async fn maybe_probe_default_daemon_socket(codex_home: &Path) -> Option<AbsolutePathBuf> {
    let socket_path = loom_app_server_client::app_server_control_socket_path(codex_home).ok()?;
...
pub(crate) async fn start_app_server_for_picker(
    ...
    let app_server = start_app_server(
        ...
        loom_tui_stubs::feedback::CodexFeedback::new(),
        /*log_db*/ None,
        state_db,
        environment_manager,
    )
...
pub async fn run_main(
    ...
    // `oss` model provider.
    let overrides_cli = loom_cli_utils::CliConfigOverrides { raw_overrides };
    ...
    let log_file = log_file_opts.open(log_dir.join("loom-tui.log"))?;
    ...
    let feedback = loom_tui_stubs::feedback::CodexFeedback::new();
...
mod tests {
    ...
    use loom_config::config_toml::ProjectConfig;
    ...
    fn resolve_remote_addr_accepts_default_socket() -> color_eyre::Result<()> {
        ...
        assert_eq!(
            ...
            RemoteAppServerEndpoint::UnixSocket {
                socket_path: loom_app_server_client::app_server_control_socket_path(&codex_home)?,
            }
        );
        Ok(())
    }
    ...
    async fn default_daemon_auto_connect_probes_socket_only() -> color_eyre::Result<()> {
        let codex_home = TempDir::new()?;
        let socket_path =
            loom_app_server_client::app_server_control_socket_path(codex_home.path())?;
    ...
    async fn fork_last_filters_latest_session_by_cwd_unless_show_all() -> color_eyre::Result<()> {
        fn write_session_rollout(
            ...
            let session_meta = loom_protocol::protocol::SessionMeta {
                id: thread_id,
            ...
            let session_meta = serde_json::to_value(loom_protocol::protocol::SessionMetaLine {
                meta: session_meta,
                git: None,
            })?;
            let lines = [
        ...
        let mut app_server = AppServerSession::new(
            loom_app_server_client::AppServerClient::InProcess(
    ...
    async fn lookup_session_target_by_name_uses_backend_title_search() -> color_eyre::Result<()> {
        Box::pin(async {
            ...
            let state_runtime = loom_tui_stubs::state::StateRuntime::init(
            ...
            let mut builder = loom_tui_stubs::state::ThreadMetadataBuilder::new(
                thread_id,
                rollout_path.clone(),
                created_at,
    ...
    async fn embedded_state_db_failure_is_typed_for_cli_recovery() -> color_eyre::Result<()> {
        ...
        assert_eq!(
            ...
            loom_tui_stubs::state::state_db_path(occupied_sqlite_home.as_path()).as_path()
        );
        assert!(
            startup_error
                .detail()
