...
use loom_app_server_protocol::ConfigReadResponse;
use loom_app_server_protocol::ConfigValueWriteParams;
use loom_app_server_protocol::ConfigWriteResponse;
use loom_app_server_protocol::FeedbackUploadParams;
use loom_app_server_protocol::FeedbackUploadResponse;
use loom_app_server_protocol::GetAccountRateLimitsResponse;
use loom_app_server_protocol::HooksListEntry;
use loom_app_server_protocol::ListMcpServerStatusParams;
use loom_app_server_protocol::ListMcpServerStatusResponse;
#[cfg(test)]
use loom_app_server_protocol::McpAuthStatus;
use loom_app_server_protocol::McpServerStatus;
use loom_app_server_protocol::McpServerStatusDetail;
use loom_app_server_protocol::MergeStrategy;
use loom_app_server_protocol::PluginInstallParams;
use loom_app_server_protocol::PluginInstallResponse;
use loom_app_server_protocol::PluginListParams;
use loom_app_server_protocol::PluginListResponse;
use loom_app_server_protocol::PluginReadParams;
use loom_app_server_protocol::PluginReadResponse;
use loom_app_server_protocol::PluginUninstallParams;
use loom_app_server_protocol::PluginUninstallResponse;
use loom_app_server_protocol::RateLimitSnapshot;
use loom_app_server_protocol::SandboxMode as AppServerSandboxMode;
use loom_app_server_protocol::SendAddCreditsNudgeEmailParams;
use loom_app_server_protocol::ServerNotification;
use loom_app_server_protocol::ServerRequest;
use loom_app_server_protocol::SkillErrorInfo;
...
use loom_config::types::MemoriesToml;
use loom_config::types::ModelAvailabilityNuxConfig;
#[cfg(target_os = "windows")]
use loom_config::types::WindowsToml;
        ...
        ServerNotification::ItemCompleted(notification) => match &notification.item {
            ThreadItem::CollabAgentToolCall { agents_states, .. } => {
                agents_states.get(receiver_thread_id).is_some_and(|state| {
                    matches!(
                        &state.status,
                        loom_app_server_protocol::CollabAgentStatus::NotFound
                    )
                })
            }
            _ => false,
...
fn session_summary(
    ...
    let resume_hint = resumable_thread.as_ref().and_then(|thread| {
        loom_cli_utils::resume_hint(thread.thread_name.as_deref(), Some(thread.thread_id))
    });
...
See the Loom keymap documentation for supported actions and examples."
            )
        })?;
        #[cfg(not(debug_assertions))]
