...
use loom_features::legacy_feature_keys;
...
pub fn features_schema(schema_gen: &mut SchemaGenerator) -> Schema {
    ...
    for feature in FEATURES {
        if feature.id == loom_features::Feature::Artifact {
            continue;
        }
        if feature.id == loom_features::Feature::MultiAgentV2 {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<loom_features::FeatureToml<
                    loom_features::MultiAgentV2ConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == loom_features::Feature::AppsMcpPathOverride {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<loom_features::FeatureToml<
                    loom_features::AppsMcpPathOverrideConfigToml,
                >>(),
            );
            continue;
        }
        if feature.id == loom_features::Feature::NetworkProxy {
            validation.properties.insert(
                feature.key.to_string(),
                schema_gen.subschema_for::<loom_features::FeatureToml<
                    loom_features::NetworkProxyConfigToml,
                >>(),
            );
            continue;
        }
