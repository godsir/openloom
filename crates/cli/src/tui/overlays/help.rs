...
impl HelpOverlay {
    ...
    fn content(&self) -> Vec<Line<'static>> {
        ...
        vec![
            ...
            cmd_line(
                "/memory persona|events|cognitions|search",
                "Query memory store",
                accent,
                dim,
            ),
            ...
            key_line(
                "Ctrl+C",
                "Quit (or cancel streaming, then quit)",
                accent,
                dim,
            ),
            ...
            key_line(
                "A/D/S/C",
                "Approve / Deny / Approve-Session / Cancel",
                accent,
                dim,
            ),
            ...
            Line::from(""),
            Line::from(Span::styled(
                " // Tip: Start message with // to send a literal /message",
                Style::new().fg(dim),
            )),
        ]
    }
