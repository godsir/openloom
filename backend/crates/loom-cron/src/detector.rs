...
//! 2. LLM extraction — parse time + body from the message
//!
...
pub fn parse_extraction_response(json_text: &str) -> Result<DetectedTask> {
    ...
    if !should_create {
        return Ok(DetectedTask { should_create: false, schedule_at: None, body: None, name: None, cron_expression: None, kind: None, confirmation: None });
    }

    let kind = v["kind"].as_str().unwrap_or("at").to_string();
    let schedule_at = v["scheduleAt"].as_str().map(|s| s.to_string());
    let body = v["reminderBody"].as_str().map(|s| s.to_string());
        ...
        _ => {
            if let Some(at) = schedule_at.as_deref() {
                if let Ok(dt) = chrono::DateTime::parse_from_rfc3339(at) {
                    Some(format!("0 {} {} {} {} {} ? {}",
                        dt.format("%M"), dt.format("%H"), dt.format("%d"),
                        dt.format("%m"), dt.format("%Y"), dt.format("%Y")))
                } else { None }
            } else { None }
        }
    };
    ...
    let confirmation = match kind.as_str() {
        "daily" => {
            ...
            format!("每天 {} 执行「{}」", t, name.as_deref().unwrap_or("定时任务"))
        }
        "interval" => {
            let m = v["everyMinutes"].as_u64().unwrap_or(60);
            format!("每 {} 分钟执行「{}」", m, name.as_deref().unwrap_or("定时任务"))
        }
        _ => {
            if let Some(at) = schedule_at.as_deref() {
                if let Ok(dt) = chrono::DateTime::parse_from_rfc3339(at) {
                    format!("{} 执行「{}」", dt.with_timezone(&chrono::Local).format("%Y-%m-%d %H:%M"), name.as_deref().unwrap_or("定时任务"))
                } else {
                    format!("「{}」", name.as_deref().unwrap_or("定时任务"))
                }
            } else {
                format!("「{}」", name.as_deref().unwrap_or("定时任务"))
            }
...
mod tests {
    ...
    #[test]
    fn test_parse_daily() {
        let json = r#"{"shouldCreate":true,"kind":"daily","timeOfDay":"08:30","scheduleAt":"2026-06-09T08:30:00+08:00","reminderBody":"每日站会","taskName":"站会提醒"}"#;
