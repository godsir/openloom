            ...
            if let Some(&next) = chars.peek() {
                if next == '[' || next == ']' {
                    ...
                    while let Some(&inner) = chars.peek() {
                        if (0x40..=0x7E).contains(&(inner as u32)) {
                            chars.next();
                            break;
                        }
                        if inner == '\x1b' || !inner.is_ascii() {
                            break;
                        }
                        chars.next();
                    }
...
pub fn convert(delta: StreamDelta) -> AppEvent {
    match delta {
        ...
        StreamDelta::ToolCallBegin { index, name, .. } => AppEvent::ToolBegin {
            index,
            name: sanitize(&name),
        },
        StreamDelta::ToolCallArgsChunk { index, chunk } => AppEvent::ToolArgsChunk {
            index,
