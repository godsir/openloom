...
impl BridgeManager {
    pub fn new() -> Self {
        ...
        Self { adapters: Arc::new(Mutex::new(HashMap::new())), inbound_rx: Arc::new(Mutex::new(rx)), inbound_tx: tx }
    }
    ...
    pub async fn send(&self, platform: Platform, chat_id: &str, content: MessageContent) -> Result<String> {
        let adapters = self.adapters.lock().await;
        adapters.get(&platform)
            .ok_or_else(|| anyhow::anyhow!("no adapter for {}", platform))?
            .send(chat_id, content).await
    }

    pub async fn health_status(&self) -> HashMap<Platform, AdapterHealth> {
        self.adapters.lock().await.iter().map(|(p, a)| (*p, a.health())).collect()
    }
