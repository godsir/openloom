...
export function connectWebSocket(port?: string, token?: string): void {
  ...
  if (!connection.port || connection.port === 0) {
    // Engine port not yet known; retry after short delay
    _wsRetryTimer = setTimeout(() => connectWebSocket(), 500);
    return;
  }
  ...
  _ws.onmessage = (event: MessageEvent) => {
    // In Electron mode, preload.js handles all WS messages via IPC bridge.
    // Avoid double-handling by skipping direct WS message dispatch when
    // the openloom preload bridge is present.
    if ((window as any).openloom) return;
    try {
