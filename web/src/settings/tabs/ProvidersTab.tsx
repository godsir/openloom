...
export function ProvidersTab() {
  const { providersSummary, selectedProviderId } = useSettingsStore(
    useShallow(s => ({ providersSummary: s.providersSummary, selectedProviderId: s.selectedProviderId }))
  );
  ...
  useEffect(() => { loadSummary(); loadProviderConfig(); }, [loadSummary, loadProviderConfig]);
    ...
    <div className={`${styles['settings-tab-content']} ${styles['active']}`} data-tab="providers">
      ...
      <SettingsSection variant="double-column">
        <div className={styles['pv-layout']}>
            ...
            {selected ? (() => {
              ...
              return (
                <ProviderDetail
                  ...
                  presetInfo={preset}
                  onRefresh={async () => { await loadProviderConfig(); await loadSummary(); }}
                />
              );
            })() : (
          ...
          {addingProvider && (
            <AddProviderOverlay
              onDone={() => { setAddingProvider(false); loadProviderConfig(); loadSummary(); }}
