...
export function lookupModelMeta(modelId: string, provider?: string): any {
  ...
  // userEntry from backend may have context:0 — don't let it clobber reference
  if ((merged as any).context === 0 && (reference as any)?.context) {
    (merged as any).context = (reference as any).context;
  }
  return merged;
}
...
export async function autoSaveConfig(
  ...
  try {
    ...
    for (const k of ['_identity', '_ishiki', '_publicIshiki', '_userProfile']) {
