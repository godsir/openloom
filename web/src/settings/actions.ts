...
export async function loadSettingsConfig() {
  ...
  try {
    ...
    const [configRes, identityRes, ishikiRes, publicIshikiRes, userProfileRes, globalModelsRes, globalConfigRes] =
      await Promise.all([
        ...
        hanaFetch('/api/config', { signal: controller.signal }),
      ]);

    const config = await configRes.json();
    const globalModels = await globalModelsRes.json();
    // Merge user-level fields from global config into agent config
    // (agent config only has agent-specific fields; user.name etc. live in global settings)
    const globalConfig = await globalConfigRes.json();
    if (globalConfig.user) config.user = globalConfig.user;
