...
import { RecordMemoryOverlay } from './overlays/RecordMemoryOverlay';
...
// If user navigates to a hidden tab, redirect to agent
...
export function SettingsContent({
  ...
  let effectiveActiveTab = normalizeNativeTabForPlatform(activeTab, platformName);
  // Redirect hidden tabs to default
  if (HIDDEN_TAB_IDS.has(effectiveActiveTab)) {
    effectiveActiveTab = DEFAULT_TAB;
  }
...
async function initSettings() {
  ...
  try {
    // openLoom: use openloom API or __enginePort__ fallback
    let serverPort = 0;
    let serverToken: string | null = null;
    if (platform?.getServerPort) {
      serverPort = Number(await platform.getServerPort());
      serverToken = await platform.getServerToken();
    } else if ((window as any).openloom?.getServerPort) {
      serverPort = Number(await (window as any).openloom.getServerPort());
    } else {
      serverPort = (window as any).__enginePort__ ?? 0;
    }
    ...
    store.set({
      ...
      serverToken,
      platformName,
      ...connectionState(createLocalServerConnection({ serverPort: String(serverPort), serverToken })),
    });
