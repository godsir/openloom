...
export interface ChatSlice {
  ...
  removeMessage: (path: string, messageId: string) => boolean;
...
export const createChatSlice = (
  ...
  appendItem: (path, item) => set((s) => {
    const session = s.chatSessions[path];
    if (!session) return {};
    // Deduplicate by message id to prevent React key conflicts
    if (item.type === 'message' && item.data?.id) {
      const alreadyExists = session.items.some(
        i => i.type === 'message' && (i as any).data?.id === item.data.id
      );
      if (alreadyExists) return {}; // silent skip — id already in list
    }
    return {
      chatSessions: {
        ...s.chatSessions,
  ...
  removeMessage: (path, messageId) => {
    ...
    set((s) => {
      const latest = s.chatSessions[path];
      if (!latest) return {};
      const items = latest.items.filter((item, i) => i !== targetIdx);
      invalidateSessionCache(path);
      invalidateStreamBuffer(path);
      return {
        chatSessions: {
