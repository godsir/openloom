...
import { yuanAvatarUrl } from '../utils/yuan-avatar-map';

const DEFAULT_NAME = 'Hanako';
const DEFAULT_VISUAL = getYuanVisual('hanako');

export function SplashApp() {
  const [avatarSrc, setAvatarSrc] = useState(yuanAvatarUrl('Hanako.png'));
      ...
      try {
        ...
        if (avatarPath && window.platform?.getFileUrl) {
          ...
          if (base) {
            ...
            setAvatarSrc(yuanAvatarUrl(getYuanVisual(splashInfo.yuan).avatar));
          }
        } else if (splashInfo?.yuan) {
          setAvatarSrc(yuanAvatarUrl(getYuanVisual(splashInfo.yuan).avatar));
        }
