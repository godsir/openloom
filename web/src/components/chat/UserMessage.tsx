...
export const UserMessage = memo(function UserMessage({
  ...
  const handleDelete = useCallback(async () => {
    if (busy || isStreaming || !message.dbId) return;
    setBusy(true);
    try {
      await deleteMessage(sessionPath, message.id, message.dbId);
    } finally {
      setBusy(false);
    }
  }, [busy, isStreaming, message.dbId, message.id, sessionPath]);
  ...
  const defaultActions: MessageFooterAction[] = useMemo(() => [
    ...
    },
    {
      ...
      disabled: isStreaming || busy || !message.dbId,
    },
  ], [busy, copied, handleCopy, handleDelete, handleEdit, handleRegenerate, isStreaming, message.dbId, t]);
          ...
          return (
            <div key={att.name || `att-${i}`} className={styles.attachImageWrap}>
              <img
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  if (att.base64Data && !/^[A-Za-z]:[\\/]/.test(att.path || '')) {
                    // Inline image from history — show in simple fullscreen overlay on body
                    e.stopPropagation();
