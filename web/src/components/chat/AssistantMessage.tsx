...
export const AssistantMessage = memo(function AssistantMessage({
  ...
  const globalAgentName = useStore(s => s.agentName) || 'Loom';
  const globalYuan = useStore(s => s.agentYuan) || 'loom';
  ...
  const handleDelete = useCallback(async () => {
    if (retrying || isStreaming || !message.dbId) return;
    setRetrying(true);
    try {
  ...
  }, [isStreaming, retrying, message.dbId, message.id, sessionPath]);
  ...
  const footerActions: MessageFooterAction[] = useMemo(() => [
    ...
    },
    {
      ...
      disabled: retrying || isStreaming || !message.dbId,
    },
  ], [handleDelete, handleRegenerate, isStreaming, message.dbId, retrying, t]);
