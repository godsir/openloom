    ...
    try {
      ...
      if (sessionHasMessages && currentSessionPath) {
        ...
        if (data.model) {
          ...
          useStore.getState().setCurrentModel({ id: data.model.id || modelId, provider: data.model.provider || provider });
        }
        ...
        let fetchedModels: Model[] = data.models || [];
        if (activeModel) {
          fetchedModels = fetchedModels.map((m: any) => ({
            ...m,
        ...
        useStore.setState({
          ...
          currentModel: activeModel ? { id: activeModel.id, provider: activeModel.provider } : null,
        });
      }
    } catch (err) {
