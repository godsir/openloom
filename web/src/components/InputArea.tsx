...
import {
  buildSlashCommands, getSlashMatches,
...
function InputAreaInner({ surface }: Required<InputAreaProps>) {
  ...
  const slashCommands = useMemo(
    () => [...buildSlashCommands(t, addToast), ...skillItems],
    [t, addToast, skillItems],
  );
  ...
  const handleDrop = useCallback(async (e: React.DragEvent) => {
    ...
    const currentCount = useStore.getState().attachedFiles.length;
    const maxAdd = 9 - currentCount;
    if (maxAdd <= 0) return;
    ...
    for (let i = 0; i < Math.min(files.length, maxAdd); i++) {
      ...
      try {
        ...
        if (upload?.dest) {
          ...
          useStore.getState().addToast(t('error.uploadFailed'), 'error');
          console.warn('[drop] upload-blob failed', upload?.error || data);
        }
      } catch (err) {
        console.warn('[drop] upload-blob error', err);
        useStore.getState().addToast(t('error.uploadFailed'), 'error');
      }
    }
  }, [addAttachedFile, t]);
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      // Guard: editor may have been destroyed while awaiting async ops above (e.g. ensureSession)
      ...
      // Add user message to the session for immediate display
      if (sessionPath) {
        const { renderMarkdown } = await import('../utils/markdown');
        // Hide welcome screen before appending — ChatArea's PanelHost won't render if welcomeVisible=true
        useStore.setState({ welcomeVisible: false });
        const userAttachments = allFiles.map(f => {
          const isImg = isImageFile(f.name);
          return {
            path: f.path,
            name: f.name || f.path.split('/').pop() || 'file',
            isDir: !!f.isDirectory,
            base64Data: isImg ? (imageBase64Map.get(f.path)?.base64Data || f.base64Data) : undefined,
            mimeType: isImg ? (imageBase64Map.get(f.path)?.mimeType || f.mimeType) : undefined,
          };
        });
        ...
        // Also update streamingSessions so InputArea's isStreaming selector fires
        const existingSessions = useStore.getState().streamingSessions || [];
        if (!existingSessions.includes(sessionPath)) {
          useStore.setState({ streamingSessions: [...existingSessions, sessionPath] });
        }
      }
      // Send via loomRpc — the backend streams the response via WebSocket events
      // (chat.stream_delta / chat.stream_end), so we don't append the assistant message here
      try {
        const loomRpc = (await import('../adapter')).loomRpc;
        ...
        if (images.length > 0) {
          rpcParams.images = images.map(img => ({
            data: img.data,
            mime_type: img.mimeType,
          }));
        }
        if (videos.length > 0) {
          rpcParams.videos = videos.map(vid => ({
            data: vid.data,
            mime_type: vid.mimeType,
          }));
        }
        if (currentModel?.id && currentModel?.provider) {
          rpcParams.model_id = currentModel.id;
          rpcParams.provider = currentModel.provider;
        }
        await loomRpc('chat.send', rpcParams);
      } catch (err: any) {
        console.error('[chat] send failed:', err);
        useStore.getState().addToast?.(err?.message || 'Send failed', 'error');
      }
    } finally {
      setSending(false);
    }
  ...
  const handleStop = useCallback(() => {
    if (!isStreaming) return;
    const sessionPath = useStore.getState().currentSessionPath;
    // In Electron mode: use loomRpc to abort via IPC bridge
    if ((window as any).openloom) {
      import('../adapter').then(({ loomRpc }) => {
        loomRpc('chat.abort', { session_id: sessionPath || 'default' }).catch(() => {});
      });
    } else {
      // Fallback: raw WebSocket abort message (non-Electron / direct WS mode)
      const ws = getWebSocket();
      if (ws) ws.send(JSON.stringify({ type: 'abort', sessionPath }));
    }
    // Optimistically clear streaming state so UI responds immediately
  ...
  return (
    ...
    >
      ...
      <div className={styles['input-stack']}>
        ...
        <div
          className={`${styles['input-wrapper']}${dragOver ? ` ${styles['dragOver']}` : ''}`}
          ref={inputCardRef}
