...
export default function ImPopoQrModal({ instanceId, instanceName, onClose, onConnected }: Props) {
  const { t } = useLocale()
  const { popoQrStart, popoQrPoll } = useIMStore()
  ...
  const startLogin = async () => {
    ...
    try {
      setStatus('loading')
      const result = await popoQrStart(instanceId)
      if (!mountedRef.current) return
      // Use qrUrl if available, otherwise show a placeholder
      setQrContent(result.qrUrl || '')
      setTaskToken(result.taskToken)
      setStatus('waiting')
      startCountdown()
      pollForScan(result.taskToken)
    } catch (err: any) {
  ...
  const pollForScan = async (token: string) => {
    const poll = async () => {
      if (!mountedRef.current) return
      try {
        const result = await popoQrPoll(token)
        if (!mountedRef.current) return
        if (result.success) {
          setStatus('connected')
          if (countdownRef.current) clearInterval(countdownRef.current)
    ...
    <div className={styles.modalOverlay} onClick={onClose}>
      <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
        <div className={styles.modalHeader}>
          <h3>POPO {t('im.qrTitle')} — {instanceName}</h3>
