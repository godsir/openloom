...
export default function InputArea() {
  ...
  // Fetch git remote URL for the current workspace
  useEffect(() => {
    if (sessionWorkspace) {
      loomRpc<{ url: string | null }>('workspace.git_remote', { workspace: sessionWorkspace })
        .then(r => { if (r.url) setGitRemoteUrl(r.url) })
        .catch(() => {})
    } else {
      setGitRemoteUrl('')
    }
        ...
        <div className={styles.composer}>
          ...
          {sessionWorkspace && (
            <div className={styles.workspaceBar}>
              ...
              {gitRemoteUrl && (
                <button
                  className={styles.workspaceGitBtn}
                  title={gitRemoteUrl}
                  onClick={() => window.loom.openExternal(gitRemoteUrl)}
                >
