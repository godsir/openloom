...
export default function ActivityPanel({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <Overlay open={open} onClose={onClose} title={t('activity.title')}>
      <div className="space-y-4 text-sm">
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-[var(--bg-card)] rounded-[var(--r-sm)] p-4 text-center border border-[var(--border)]">
            ...
            <p className="text-[11px] font-mono text-[var(--text-muted)] mt-1 uppercase tracking-wide">{t('activity.totalAgents')}</p>
          </div>
          <div className="bg-[var(--bg-card)] rounded-[var(--r-sm)] p-4 text-center border border-[var(--border)]">
            <p className="font-display text-2xl text-[var(--green)]">{streamingCount}</p>
            <p className="text-[11px] font-mono text-[var(--text-muted)] mt-1 uppercase tracking-wide">{t('activity.activeStreams')}</p>
          </div>
        </div>
        ...
        {activeAgents.length === 0 && streamingCount === 0 && (
          <p className="text-[var(--text-muted)] text-center py-6">{t('activity.empty')}</p>
        )}
      </div>
    </Overlay>
  )
